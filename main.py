...
def run_api():
    ...
    try:
        ...
        print(f"Error: {e}")
        print(
            "Make sure models/dnn/api_server.py exists and all dependencies are installed."
        )
        sys.exit(1)
...
def run_inference(user_id, user_text=None, num_recommendations=10):
    ...
    try:
        ...
        print(f"Error: {e}")
        print(
            "Make sure models/dnn/inference_v2.py exists and all dependencies are installed."
        )
        sys.exit(1)
...
def main():
    ...
    inference_parser = subparsers.add_parser(
        "recommend", help="Get recommendations for a user"
    )
    inference_parser.add_argument("--user_id", type=int, help="User ID")
    inference_parser.add_argument(
        "--user_text", type=str, help="User profile text (for cold start)"
    )
    inference_parser.add_argument(
        "--num", type=int, default=10, help="Number of recommendations"
    )
