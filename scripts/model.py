...
class RecommendationModel:
    ...
    def load_data(self):
        ...
        self.user_llm_emb = torch.tensor(
            user_llm_data["embedding"].tolist(), dtype=torch.float32
        )
        self.item_llm_emb = torch.tensor(
            item_llm_data["embedding"].tolist(), dtype=torch.float32
        )
        ...
        print(
            f"Loaded data for {len(self.user_ids)} users and {len(self.item_ids)} items."
        )
    ...
    def load_fold_data(self, fold_id):
        ...
        # Map IDs to indices
        train_data["user_idx"] = train_data["user_id"].map(
            lambda x: self.user_id_map[str(x)]
        )
        train_data["item_idx"] = train_data["item_id"].map(
            lambda x: self.item_id_map[str(x)]
        )
        test_data["user_idx"] = test_data["user_id"].map(
            lambda x: self.user_id_map[str(x)]
        )
        test_data["item_idx"] = test_data["item_id"].map(
            lambda x: self.item_id_map[str(x)]
        )
    ...
    def train_final_model(self, epochs=30):
        ...
        self.ratings["user_idx"] = self.ratings["user_id"].map(
            lambda x: self.user_id_map[str(x)]
        )
        self.ratings["item_idx"] = self.ratings["item_id"].map(
            lambda x: self.item_id_map[str(x)]
        )
