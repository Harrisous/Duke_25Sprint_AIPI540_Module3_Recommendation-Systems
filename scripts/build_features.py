...
def generate_user_profiles():
    ...
    movies = pd.read_csv(movies_file, sep="|", names=column_names, encoding="latin-1")

    # Load genres
    genres = pd.read_csv(
        genres_file, sep="|", names=["genre", "genre_id"], encoding="latin-1"
    )
    ...
    for user_id in tqdm(users["user_id"].unique()):
        ...
        # Store user profile
        user_profiles.append({"user_id": str(user_id), "llm_text": profile_text})
...
def generate_movie_profiles():
    ...
    for _, movie in tqdm(movies.iterrows(), total=len(movies)):
        ...
        movie_profiles.append(
            {"item_id": str(movie["movie_id"]), "llm_text": profile_text}
        )
