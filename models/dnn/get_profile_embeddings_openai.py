...
from openai import (OpenAI,  # Use OpenAI client and specific error
                    RateLimitError)
...
try:
    ...
    print(
        "Ensure 'get_text_profiles.py' was run successfully and files are in the correct 'processed' directory."
    )
    exit()
...
@retry(
    ...
    retry=retry_if_exception_type(
        RateLimitError
    ),  # Retry specifically on OpenAI RateLimitError
    before_sleep=lambda retry_state: print(
        f"OpenAI rate limit hit, waiting {retry_state.next_action.sleep:.2f} seconds..."
    ),
)
def get_openai_embedding(text: str) -> List[float] | None:  # Return type hint updated
    ...
    try:
        ...
        print(
            f"An error occurred during OpenAI embedding generation for text snippet '{text[:50]}...': {e}"
        )
...
def process_with_progress(df: pd.DataFrame, desc: str) -> pd.DataFrame:
    ...
    if len(df) < initial_count:
        print(
            f"Warning: Dropped {initial_count - len(df)} rows from '{desc}' due to embedding errors."
        )
    return df
...
if __name__ == "__main__":
    ...
    user_texts_with_embeddings = process_with_progress(
        user_texts, "Processing user data with OpenAI"
    )
    movie_texts_with_embeddings = process_with_progress(
        movie_texts, "Processing movie data with OpenAI"
    )

    # Save embeddings (consider adding indent for readability)
    user_output_path = (
        PROCESSED_DATA_DIR / "user_embeddings_openai.json"
    )  # Save with a new name
    movie_output_path = (
        PROCESSED_DATA_DIR / "movie_embeddings_openai.json"
    )  # Save with a new name
