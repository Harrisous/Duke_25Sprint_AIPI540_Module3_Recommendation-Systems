...
# Load data
ratings = pd.read_csv(
    RAW_DATA_DIR / "ml-100k/u.data",
    sep="\t",
    names=["user_id", "item_id", "rating", "timestamp"],
)
users = pd.read_csv(
    RAW_DATA_DIR / "ml-100k/u.user",
    sep="|",
...
movies = pd.read_csv(
    RAW_DATA_DIR / "ml-100k/u.item",
    sep="|",
    encoding="latin-1",
    header=None,
    names=movie_columns,
    usecols=range(5 + len(genres)),
)
...
user_history = (
    ...
    .apply(
        lambda x: [
            f"'{row['title']}' (Genre: {', '.join(row['genres'])}) rated {int(row['rating'])} stars"
...
movie_texts.loc[:, "item_idx"] = movie_texts["item_id"].map(
    lambda x: item_id_map[str(x)]
)
