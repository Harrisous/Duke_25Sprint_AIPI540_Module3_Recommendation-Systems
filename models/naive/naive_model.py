...
class NaiveRecommendation:
    ...
    def _data_loader(self):
        ...
        file_path = os.path.join(current_dir, "..", "..", "data", "raw", "ml-100k", "u.data")
        ...
        for user_id, item_id, rating, _ in data: 
            ratings_matrix[user_id - 1, item_id - 1] = rating  # user id and movie id starts from 1

        # Print the shape of the resulting matrix
        print(f"Load complete, ratings matrix loaded shape: {ratings_matrix.shape}") # shape: (943,1682)
...
def random_user_vector(vector_length = 1682, valid_size = 5):
    ...
    random_indices = np.random.choice(vector_length, size=valid_size, replace=False) # random places
    vector[random_indices] = np.random.randint(1, 6, size=valid_size) # random rates (0-5) on random places
    return vector
